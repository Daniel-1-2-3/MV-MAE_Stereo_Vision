...
from Custom_Mujoco_Playground._src.dm_control_suite import common

_XML_PATH = mjx_env.ROOT_PATH / "dm_control_suite" / "xmls" / "ball_in_cup.xml"
...
class BallInCup(mjx_env.MjxEnv):
  """Ball in cup environment."""
  ...
  def _post_init(self) -> None:
    ...
    self._target_size = self._mj_model.site_size[self._target_site_id, [0, 2]]
    geom_id = self._mj_model.geom("ball").id
  ...
  def _get_reward(
    ...
    ball_to_target = jp.abs(target - ball)
    inside = jp.where(
        ball_to_target < self._target_size - self._ball_size, 1.0, 0.0
    )
    return jp.prod(inside, axis=-1)
