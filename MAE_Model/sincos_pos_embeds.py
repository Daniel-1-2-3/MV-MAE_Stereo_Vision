...
class PosEmbed():
    ...
    def get_2d_sincos_pos_embed(embed_dim, grid_h_size, grid_w_size):
        ...
        """
        grid_h = jnp.arange(grid_h_size, dtype=jnp.float32)
        grid_w = jnp.arange(grid_w_size, dtype=jnp.float32)
        grid = jnp.meshgrid(grid_w, grid_h)  # (x, y), each (H, W)
    ...
    def get_2d_sincos_pos_embed_from_grid(embed_dim, grid):
        ...
        Args:
            embed_dim (int): Embedding dimension for each patch
            grid (array): Array of shape (2, height, width), where grid[0]
                                is the x_coordinates of all cells in the grid, while
                                grid[1] is the y_coordinates of all cells in the grid
        """
        # Keep the same constraint; for JIT you should pass embed_dim as a Python int (static)
        ...
        emb = jnp.concatenate([embed_h, embed_w], axis=1) # (M, embed_dim)
        return emb
    ...
    def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
        ...
        Args:
            embed_dim (int): Embedding dimension for each patch
            pos (array):  A list of positions to be encoded (H, W), could be
                                x or y coordinates
        """
        assert embed_dim % 2  == 0
        # Frequency spectrum
        ...
        out = jnp.einsum("m,d->md", pos, omega)  # (M, embed_dim / 2), outer product

        emb_sin = jnp.sin(out) # (M, embed_dim / 2)
        emb_cos = jnp.cos(out) # (M, embed_dim / 2)

        emb = jnp.concatenate([emb_sin, emb_cos], axis=1) # (M, embed_dim)
        return emb
