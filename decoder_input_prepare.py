...
class DecoderInputPreparation(nn.Module):
    def __init__(self, batch_size, img_size, patch_size, encoder_embed_dim, decoder_embed_dim):
        super().__init__()
        ...
        self.pos_embeddings = nn.Parameter(
            torch.randn(batch_size, self.num_patches * 2, decoder_embed_dim) 
        ) # Decoder input after concatenation has this shape
    
    def forward(self, x: Tensor):
        x = self.change_dim(x)
        mask_tokens = self.mask_token.expand(self.batch_size, self.num_patches, -1)
