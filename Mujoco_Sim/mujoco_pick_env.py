...
class StereoPickCube(pick.PandaPickCube):
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        if self._vision:
            ...
            img_left = adjust_brightness(jp.asarray(rgb[0, 0, ..., :3], dtype=jp.float32) / 255.0, brightness).astype(jp.uint8)
            img_right = adjust_brightness(jp.asarray(rgb[1, 0, ..., :3], dtype=jp.float32) / 255.0, brightness).astype(jp.uint8)
            fused = jp.concatenate([img_left, img_right], axis=1).astype(jp.uint8)
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        newly_reset = info["_steps"] == jp.asarray(0, jp.int32) # Episode-start flag (used for exploration)
